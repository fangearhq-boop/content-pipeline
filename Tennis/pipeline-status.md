...
| Writing | Complete (all steps) | 2026-05-17 |
| Fact-check | Complete (verify-facts.py passed — 5 stories, 31 claims, image warning cosmetic) | 2026-05-17 |
| Compile | Complete (07-content-data.json — 5 stories, 7 X posts, 5 FB posts, 5 articles, 10 images) | 2026-05-17 |
| Dashboard | Complete (review-dashboard.html, 27 items) | 2026-05-17 |
| PostPlanner Export | Complete (standard 7 posts + TOBI 7 posts) | 2026-05-17 |
| WordPress Publish | Attempted — proxy blocks fanrumor.com (same as all previous runs) | 2026-05-17 |
| Dashboard Push | Attempted — PAT lacks write access to content-dashboards repo (same as all prior runs) | 2026-05-17 |
...
- **PostPlanner exports:** tfr-postplanner-2026-05-16.xlsx (7 posts), tfr-postplanner-tobi-2026-05-16.xlsx (7 TOBI posts)
- **Coverage:** Sinner leads Medvedev 6-2, 5-7, 4-2 (rain halted Rome SF; resumes today 3 PM; Career Golden Masters 2 games away; Djokovic only prior CGM holder); Ruud def. Darderi 6-1, 6-1 (5th Rome final; Ruud "revenge" vs Sinner; Medvedev 2023 Rome champ = potential rematch); WTA Rome Final matchday Gauff vs Svitolina (today 5 PM local; Svitolina 7-0 in WTA clay finals; Gauff never won Rome; first clay meeting); Ruud's Rome curse (5 finals without title, 0-4 before 2026; 3x GS finalist); RG qualifying draw Saturday May 17, qualifying starts Monday May 18 (wild cards Wawrinka/Monfils/Tabur; Alcaraz out; Sinner heavy favourite)
- **Notes:**
  - verify-facts.py passed — all 5 stories present; 39 claims; image warnings cosmetic (not_started expected)
  - compile-content-data.py: 5 stories, 7 X posts, 5 FB posts, 5 articles, 9 images
  - review-dashboard.html generated (27 items)
  - PostPlanner exports: both standard (7) and TOBI (7) generated successfully
  - WordPress proxy error: same environment restriction as all prior runs
  - Dashboard push failed: PAT lacks write access to content-dashboards repo (same as all prior runs)

### 2026-05-15 — Full Pipeline Run
- **Steps completed:** All 15 (Research → Story History → Brief → Research Notes → Analysis → X Posts → FB Posts → Image Concepts → Articles → Fact-Check → Compile → Image Manifest → Dashboard → PostPlanner Export x2 → Publish attempt)
- **Stories:** 5 stories (3 Tier 1, 2 Tier 2)
- **Posts:** 8 X posts + 10 FB posts (5 long-form + 5 captions) = 18 total
- **Articles:** 5 (bylines: Elena Voss x2, Marcus Cole x2, Ryan Calloway x1)
- **PostPlanner exports:** tfr-postplanner-2026-05-15.xlsx (8 posts), tfr-postplanner-tobi-2026-05-15.xlsx (8 TOBI posts)
- **Coverage:** Svitolina def. Swiatek 6-4, 4-6, 6-2 WTA SF (Swiatek won 88-86 pts, still lost; Svitolina BP conversion 67% vs 31%; beats Nos. 2+4 in 24 hours; first Rome final since 2018 title); WTA Rome Final preview Gauff vs Svitolina (Svitolina leads H2H 3-2, all on hard; FIRST career clay meeting; RG qualifying starts May 18); ATP SF Day Sinner vs Medvedev (32 M1000 wins = all-time record; Career Golden Masters — Rome only M1000 Sinner hasn't won; Medvedev 2023 Rome champion; Darderi vs Ruud other SF); Darderi Italian homecoming (wild card 23yo; beat Zverev R4; 2 AM QF finish vs Jodar; first career Masters SF at home in Rome; faces Ruud 4x finalist); RG countdown 3 days (qualifying May 18; draw May 21; main draw May 24; Wawrinka + Monfils wild cards; Alcaraz out)
- **Notes:**
  - verify-facts.py passed — all 5 stories present; 26 claims; image warnings cosmetic (not_started expected)
  - compile-content-data.py: 5 stories, 8 X posts, 5 FB posts, 5 articles, 9 images
  - review-dashboard.html generated (28 items)
  - PostPlanner exports: both standard (8) and TOBI (8) generated successfully
  - WordPress proxy error: same environment restriction as all prior runs
  - Dashboard push failed: PAT lacks write access to content-dashboards repo (same as all prior runs)

### 2026-05-14 — Full Pipeline Run
- **Steps completed:** All 15 (Research → Story History → Brief → Research Notes → Analysis → X Posts → FB Posts → Image Concepts → Articles → Fact-Check → Compile → Image Manifest → Dashboard → PostPlanner Export x2 → Publish attempt)
- **Stories:** 5 stories (3 Tier 1, 2 Tier 2)
- **Posts:** 7 X posts + 10 FB posts (5 long-form + 5 captions) = 17 total
- **Articles:** 5 (bylines: Elena Voss x2, Marcus Cole x2, Ryan Calloway x1)
- **PostPlanner exports:** tfr-postplanner-2026-05-14.xlsx (7 posts), tfr-postplanner-tobi-2026-05-14.xlsx (7 TOBI posts)
- **Coverage:** Sinner vs Rublev ATP Rome QF (31 consecutive M1000 wins tied Djokovic all-time record; match in progress, Sinner leading 6-2 Set 1; win = sole record + Career Golden Masters; Rome only M1000 Sinner hasn't won); Svitolina def. Rybakina 2-6, 6-4, 6-4 (saved 16 break points; first Rome SF since 2018 title; faces Swiatek); WTA Rome SF Day (Swiatek vs Svitolina + Gauff vs Cirstea; RG 10 days away); Darderi def. Jodar 6-7, 7-5, 6-0 (match suspended by Coppa Italia smoke from Olimpico; Darderi first Masters SF; Ruud def. Khachanov 6-1, 1-6, 6-2); Roland Garros countdown (qualifying May 18; draw May 21; main draw May 24; Wawrinka + Monfils wild cards)
- **Notes:**
  - verify-facts.py passed — all 5 stories present; 36 claims; image warnings cosmetic (not_started expected)
  - compile-content-data.py: 5 stories, 7 X posts, 0 FB (format parse issue, same as all prior runs), 5 articles, 10 images
