...
- Byline: "By {BYLINE} | {Date}" — rotate through names from `article_bylines` in niche-config.yaml
...
**Photo/Figure:**
- Featured image is generated by Gemini and attached via WordPress API (not embedded in article HTML)
...
- [ ] No `<figure>` block in article HTML (featured image handled by WordPress)
