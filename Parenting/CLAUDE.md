...
- [ ] Exclamation marks: max 1 per social post, 0 in articles
- [ ] Platform rules matched (X: 280 chars, 4 hashtags; FB: no hashtags, engagement question; Articles: 500-1000 words)
- [ ] Every story maps to one of the 5 content pillars
- [ ] Gemini image prompts: mode is `base_only`, clean bottom third for text overlay, no celebrity likenesses
- [ ] Canva brand kit ID `kAHCKfCZgk0` used for designs
- [ ] localStorage keys use `parenting-content-` prefix
- [ ] Fact-check priority levels respected (HIGH for prices/free claims/ages/phones/addresses)
- [ ] File naming: `parenting-content-YYYY-MM-DD/` folders, `article-NN-slug-title.html` articles

## Decision Log
Append-only. Record WHY, not just WHAT.

- **2026-02-23:** Gemini `base_only` mode chosen over `complete` — Canva handles text overlays for brand consistency across all image types.
- **2026-02-23:** Model default is `gemini-2.5-flash-image` (~$0.04/image) — cost-effective for daily 7-story volume. Pro model only on explicit request.
